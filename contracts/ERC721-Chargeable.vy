...
@param _beneficiary Address which funds will be withdrawn to
  ...
  self.maintenance_wallet = maintenance_wallet
...
@external
def changeMaintenanceWallet(to: address) -> bool:
  assert msg.sender == self.maintenance_wallet, "Only the maintenance wallet owner can change the wallet address."
  self.maintenance_wallet = to
  self.beneficiary = to
  return True
...
@payable
@external
def mint(_to: address, _tokenURI: String[64], maintenanc_fee: uint256) -> bool:
  assert msg.value >= maintenance_fee / 10 ** 18, "Insufficient payment for minting"

  # Calculate the amount to be sent to the maintenance wallet
  maintenance_amount = msg.value

  # transfer maintenance fee to website owner
  send(self.maintenance_wallet, maintenance_amount)
